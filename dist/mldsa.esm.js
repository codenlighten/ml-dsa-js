/*! ml-dsa-browser-cdn v0.2.0 | (c) 2026 Gregory J. Ward, CTO SmartLedger.Technology | MIT License | https://github.com/codenlighten/ml-dsa-js#readme */
...
function ecdh(Point2, ecdhOpts = {}) {
  ...
  return Object.freeze({ getPublicKey, getSharedSecret, keygen: keygen2, Point: Point2, utils: utils3, lengths });
}
function ecdsa(Point2, hash, ecdsaOpts = {}) {
  ...
  const { keygen: keygen2, getPublicKey, getSharedSecret, utils: utils3, lengths } = ecdh(Point2, ecdsaOpts);
...
function keygenFromMnemonic(options = {}) {
  const {
    mnemonic,
    passphrase = "",
    level = 65,
...
function ecdsaKeygenFromMnemonic(options = {}) {
  const {
    mnemonic,
    passphrase = "",
    ...
    change = 0,
    index = 0,
    path = defaultEcdsaPath(chain2, account, change, index)
  } = options;
  const { node } = deriveNodeFromMnemonic(mnemonic, passphrase, path);
...
function ecdsaVerify(signature, message, publicKey, options = {}) {
  const { hash = "sha256" } = options;
  const digest = hashForEcdsa(message, hash);
  const sig = normalizeBytes(signature, "signature");
  const pk = normalizeBytes(publicKey, "publicKey");
...
function deriveDualStackFromMnemonic(options = {}) {
  const {
    mnemonic,
    passphrase = "",
    chain: chain2 = "ethereum",
